...
from stable_baselines3.common.buffers import RolloutBufferWithCost
from stable_baselines3.common.on_policy_algorithm import OnPolicyAlgorithmWithCost
from stable_baselines3.common.policies import ActorTwoCriticCnnPolicy, ActorTwoCriticPolicy, BasePolicy, MultiInputActorTwoCriticPolicy
from stable_baselines3.common.type_aliases import GymEnv, MaybeCallback, Schedule
from stable_baselines3.common.utils import explained_variance, get_schedule_fn
from stable_baselines3.common.dual_variable import DualVariable, PIDLagrangian
SelfPPOLag = TypeVar("SelfPPOLag", bound="PPOLagrangian")


class PPOLagrangian(OnPolicyAlgorithmWithCost):
    """
    ...
    policy_aliases: ClassVar[Dict[str, Type[BasePolicy]]] = {
        "MlpPolicy": ActorTwoCriticPolicy,
        "CnnPolicy": ActorTwoCriticCnnPolicy,
        "MultiInputPolicy": MultiInputActorTwoCriticPolicy,
    }
    ...
    def _setup_model(self) -> None:
        ...
        if self.clip_range_cost_vf is not None:
            if isinstance(self.clip_range_cost_vf, (float, int)):
                assert self.clip_range_cost_vf > 0, "`clip_range_cvf` must be positive, " "pass `None` to deactivate cvf clipping"
    ...
    def train(self) -> None:
        ...
        self.logger.record("train/reward_value_loss", np.mean(reward_value_losses))
        self.logger.record("train/cost_value_loss", np.mean(cost_value_losses))
        self.logger.record("train/approx_kl", np.mean(approx_kl_divs))
        self.logger.record("train/clip_fraction", np.mean(clip_fractions))
        self.logger.record("train/loss", loss.item())
        self.logger.record("train/mean_reward_advantages", mean_reward_advantages)
        self.logger.record("train/mean_cost_advantages", mean_cost_advantages)
        self.logger.record("train/reward_explained_variance", explained_reward_var)
        self.logger.record("train/cost_explained_variance", explained_cost_var)
        self.logger.record("train/nu", self.dual.nu().item())
        self.logger.record("train/nu_loss", self.dual.loss.item())
        self.logger.record("train/average_cost", average_cost)
        self.logger.record("train/total_cost", total_cost)
