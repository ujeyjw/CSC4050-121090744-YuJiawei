...
class ActorTwoCriticPolicy(BasePolicy):
    """
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        if self.share_features_extractor:
            ...
            latent_vf,latent_cvf = self.mlp_extractor.forward_critic(vf_features, cvf_features)
    ...
    def predict_values(self, obs: PyTorchObs) -> th.Tensor:
        ...
        # breakpoint()
        latent_vf, latent_cvf = self.mlp_extractor.forward_critic(vf_features,cvf_features)
        return self.value_net(latent_vf), self.cost_value_net(latent_cvf)
...
class ActorTwoCriticCnnPolicy(ActorTwoCriticPolicy):
    def __init__(
        self,
        observation_space: spaces.Space,
        action_space: spaces.Space,
...
class MultiInputActorTwoCriticPolicy(ActorTwoCriticPolicy):
    """
    MultiInputActorClass policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
