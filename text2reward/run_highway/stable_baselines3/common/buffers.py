...
from stable_baselines3.common.type_aliases import (
    ...
    RolloutBufferSamplesWithCost,
    DictRolloutBufferSamplesWithCost
)
...
class RolloutBufferWithCost(BaseBuffer):
    ...
    def compute_returns_and_advantage(self, reward_last_value: th.Tensor, cost_last_value: th.Tensor,
        ...
        self.reward_returns, self.reward_advantages = self._compute_returns_and_advantage(
                self.rewards, self.reward_values, self.episode_starts, self.reward_gamma, self.reward_gae_lambda,
                reward_last_value, dones, self.reward_advantages
        )
        self.cost_returns, self.cost_advantages = self._compute_returns_and_advantage(
                self.costs, self.cost_values, self.episode_starts, self.cost_gamma, self.cost_gae_lambda,
    ...
    def _get_samples(
        ...
        return RolloutBufferSamplesWithCost(*tuple(map(self.to_torch, data)))
...
class DictRolloutBufferWithCost(RolloutBufferWithCost):
    """
    Dict Rollout buffer used in on-policy algorithms like A2C/PPO.
    Extends the RolloutBuffer to use dictionary observations
    ...
    def get(  # type: ignore[override]
        ...
        if not self.generator_ready:
            ...
            _tensor_names = ["actions", "log_probs", "reward_values",
    ...
    def _get_samples(  # type: ignore[override]
        ...
        return DictRolloutBufferSamplesWithCost(
            observations={key: self.to_torch(obs[batch_inds]) for (key, obs) in self.observations.items()},
            actions=self.to_torch(self.actions[batch_inds]),
            old_log_prob=self.to_torch(self.log_probs[batch_inds].flatten()),
            old_reward_values=self.to_torch(self.reward_values[batch_inds].flatten()),
            reward_advantages=self.to_torch(self.reward_advantages[batch_inds].flatten()),
            reward_returns=self.to_torch(self.reward_returns[batch_inds].flatten()),
