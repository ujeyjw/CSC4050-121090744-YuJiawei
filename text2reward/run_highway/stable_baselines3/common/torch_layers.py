...
class MlpExtractor(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(net_arch, dict):
            ...
            if create_cvf:
                cvf_layers_dims = net_arch.get("cvf", [])
        else:
    ...
    def forward(self, features: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        ...
        """
        if self.create_cvf:  
            vn, cvn = self.forward_critic(features,features)
            return self.forward_actor(features), vn, cvn
        else :
    ...
    def forward_critic(self, vfeatures: th.Tensor, cvfeatures: th.Tensor=None) -> th.Tensor:
        if self.create_cvf:
            return self.value_net(vfeatures), self.cost_value_net(cvfeatures)
        else:
            return self.value_net(vfeatures)
