...
class VecNormalizeWithCost(VecNormalize):
    ...
    def __init__(
        ...
        clip_obs=10.0, clip_reward=10.0, clip_cost=10.0, reward_gamma=0.99, cost_gamma=0.99, epsilon=1e-8, norm_obs_keys: Optional[List[str]] = None,
    ):
        super().__init__(venv=venv, training=training, norm_obs=norm_obs, norm_reward=norm_reward,
                         clip_obs=clip_obs, clip_reward=clip_reward, gamma=reward_gamma,
                         epsilon=epsilon, norm_obs_keys=norm_obs_keys)
    ...
    def step_wait(self):
        ...
        obs, rews, done, infos = super().step_wait()
